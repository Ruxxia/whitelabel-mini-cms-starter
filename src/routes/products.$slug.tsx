...
function ProductDetail() {
  ...
  useSEO({ title: p?.name ?? "Product", description: p?.description || undefined, image: p?.image_url || undefined });

  if (isLoading) return <SiteLayout><div className="container mx-auto p-8">Loading...</div></SiteLayout>;
  if (!p) return <SiteLayout><div className="container mx-auto p-8">Product not found.</div></SiteLayout>;
        ...
        <article className="container mx-auto px-4 py-10 max-w-4xl">
          ...
          <div className="mt-6 grid gap-8 md:grid-cols-2">
            ...
            <div>
              ...
              {p.price && <p className="mt-3 text-2xl font-bold text-primary">${Number(p.price).toLocaleString("en-US")}</p>}
